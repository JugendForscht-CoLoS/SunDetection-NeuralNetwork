...
#Start of cnn
base_model = tf.keras.applications.MobileNetV2(input_shape=[PICTURE_SIZE, PICTURE_SIZE, 3], include_top=False)
...
layers = [base_model.get_layer(name).output for name in layer_names] #Die layers von layer_names bekommen die Ausgabe des MobileNetV2
...
def unet_model(output_channels):
  inputs = tf.keras.layers.Input(shape=[PICTURE_SIZE, PICTURE_SIZE, 3])
  x = inputs
